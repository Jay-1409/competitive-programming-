...
using namespace std;
void printseq(string cmd, string ip, string nm){
    cout<<cmd<<" "<<ip<<';'<<" "<<'#'<<nm<<endl;
}
void solve() {
    int n,m; scd(n);scd(m);
    vector<pair<string,string>>ser;
    vector<pair<string,string>>cmd;
    string x,y;
    f0r(i,n){
        cin>>x>>y;
        ser.push_back(make_pair(x,y));
    }
    f0r(i,m){
        cin>>x>>y;
        cmd.push_back(make_pair(x,y));
        cmd[i].second.pop_back();
    }
    f0r(i,m){
        f0r(j,n){
            if(cmd[i].second==ser[j].second){
                printseq(cmd[i].first,ser[j].second,ser[j].first);
            }
        }
    }
}
