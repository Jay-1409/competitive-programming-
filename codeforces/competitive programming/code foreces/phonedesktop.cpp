...
void solve() {
    ...
    cin>>x>>y;
    int ans;
    // one screen can have max 2 2x2 icons
    if(x==0&&y==0){
        cout<<0<<endl; return;
    }
    int space2=ceil(y/(double)2);
    if(space2==0){
        ans=ceil(x/(double)15);
    }else{
        int extra;
        int empty;
        int totalnumberof2x2=space2*15;
        empty=totalnumberof2x2-(y*4);
        int space1;
        if(x>empty){
            extra=abs(empty-x);
            space1=ceil(extra/(double)15);
        }else{
            space1=0;
        }
        ans=space1+space2;
