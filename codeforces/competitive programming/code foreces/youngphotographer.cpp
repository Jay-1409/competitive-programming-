...
void solve() {
    ...
    if(n==1){
        int a,b; cin>>a>>b;
        if(a<=x&&b>=x){cout<<0<<endl;return;}
        cout<<min(abs(x-a),abs(x-b))<<endl;return;
    }
    f0r(i,n){
        cin>>a>>b;
        an.push_back(a);
    ...
    f0r(i,1,n){
        if((an[i]>an[i-1])&&(bn[i]>bn[i-1])){
            cout<<-1<<endl;return;
        }
    }
    cout<<min(abs(x-maxa),abs(minb-x))<<endl;
    // int dep=maxa-x,pep=x-minb;
    // if(maxa-x<0){
    //     dep=0;
    // }
    // if(x-minb<0){
...
void solve2()
{
    ...
    if (minx <= x && x <= miny)
        ...
        cout << min(abs(x - minx), abs(x - miny)) << endl;
}
int main() {
