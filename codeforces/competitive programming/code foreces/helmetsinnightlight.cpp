...
void solve() {
    ...
    f0r(i,n){
        cin>>temp1>>temp2;
        pr.push_back(make_pair(temp1,temp2));
    }
    sort(pr.begin(),pr.end(),sortBySecond);
    int reu=0; int i=0;
    while(n>0){
        if(i==0){reu+=p;n--;}
        // cout<<pr[i].first<<" "<<pr[i].second<<endl;
        if(pr[i].second*pr[i].first<p){
            cout<<"t"<<endl;            
            reu+=pr[i].first*pr[i].second;
            n-=pr[i].first;
        }else{
            cout<<"s"<<endl;
            reu+=p;
...
void solve2(){
    ...
    while(n>0){
        cout<<n<<" "<<reu<<endl;
        // if(i==0){reu+=p;n-=1;i++;continue;}
        if(pr[i].first>=p){
            reu+=(i+1)*p;
            n--;
            break;
        }
        int x=min(i+1,pr[i].second);
        reu+=pr[i].second*x;
        n-=x;
        i++;
        if(i+1<n){
