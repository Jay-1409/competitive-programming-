...
void solve() {
    ...
    f0r(i,n){
      cin>>tempa>>tempb;
      v.push_back({tempa,tempb});
      track[tempa]=i;
    }
    f0r (i,n) {
        int xi = v[i].first;
        int di = v[i].second;
        int target_position = xi + di;
        if (track.find(target_position) != track.end()) {
            int j = track[target_position];
            int reverse_target_position = v[j].first + v[j].second;
            if (reverse_target_position == xi) {
                cout << "YES" << endl;
                return;
            }
        }
