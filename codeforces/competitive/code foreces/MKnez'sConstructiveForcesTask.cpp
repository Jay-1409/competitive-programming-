...
void solve() {
    ...
    if(n==3){
        ...
        f0r(i,n){
            if(i&1){cout<<-place<<" ";}else cout<<abs(place)-1<<" ";
        }cout<<endl;
    }else{
        cout<<yes<<endl;
        f0r(i,n){
            if(i&1)cout<<1<<" ";else cout<<-1<<" ";
        }
        cout<<endl;
    }
    /*
    ...
    if n is odd
    n=3 --> not possible
    n=5 --> not possible , why ? cause possible sum be -1
    array can be 1 -2 1 -2 1 
    total eal sum= -1
    individual sum = -1 -1 -1 -1 what ?? possible?? eh yea
    for 3 then --> total possible sum =-1
    1 -2 1
    cause if(n>=5 then there will always be an answer but for n==3 there are cases where ans is not possible)
    for n>5 &1 
    -1 1 -1 1 -1 1 -1
    works wao lol nope haha jk
    for n==5 we did 
    1 -2 1 -2 ..
    for n==7 we can do
    2 -3 2 -3 2 -3 2
