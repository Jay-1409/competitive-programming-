...
void findPositions(set<int> &current_positions, vector<pair<int, char>> &throws, int n) {
    set<int> new_positions;
    for (int i = 0; i < throws.size(); ++i) {
        new_positions.clear();
        int dist = throws[i].first;
        char direction = throws[i].second;
        for (int pos : current_positions) {
            if (direction == '0' || direction == '?') {
                int new_pos = (pos + dist - 1) % n + 1; // Clockwise
                new_positions.insert(new_pos);
            }
            if (direction == '1' || direction == '?') {
                int new_pos = (pos - dist - 1 + n) % n + 1; // Counterclockwise
                new_positions.insert(new_pos);
            }
        }
...
void solve() {
    ...
    while (t--) {
        ...
        set<int> current_positions = {x};
        findPositions(current_positions, throws, n);

        vector<int> result(current_positions.begin(), current_positions.end());
