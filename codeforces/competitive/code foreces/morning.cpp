...
void solve() {
    ...
    f0r(i,len(s)){
        // cout<<s[i]<<" "<<clk<<endl;
        // cout<<(((int)s[i]-'0')-curd)<<endl;
            // cout<<s[i]<<" "<<curd<<endl;
        if(s[i]=='0'){
            // cout<<abs((int)s[i]-'9')-curd+1<<endl;
            clk+=abs((int)s[i]-'9'-curd);
            clk++;
            curd=(int)s[i]-'0';continue;
        }
        else if(curd==0){
            clk+=abs((int)s[i]-'9');
            clk++;
...
void againsolve(){
    ...
    if(s[1]=='0'){
        ans+=abs(10-((int)s[0]-'0'));
    }else{
        ans+=abs(((int)s[1]-'0')-((int)s[0]-'0'));
    }
    if(s[2]=='0'){
        ans+=abs(10 - ((int)s[1]-'0'));
    }else{
        ans+=abs(((int)s[2]-'0') - ((int)s[1]-'0'));
    }
    if(s[3]=='0'){
        ans+=abs(10 - ((int)s[2]-'0'));
    }else{
        ans+=abs(((int)s[3]-'0') - ((int)s[2]-'0'));
    }
    cout<<ans<<endl;
}
int main() {
