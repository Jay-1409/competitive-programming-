...
void __print(int x) { cerr << x; } void __print(long x) { cerr << x; } void __print(long long x) { cerr << x; } void __print(unsigned x) { cerr << x; } void __print(unsigned long x) { cerr << x; } void __print(unsigned long long x) { cerr << x; } void __print(float x) { cerr << x; } void __print(double x) { cerr << x; } void __print(long double x) { cerr << x; } void __print(char x) { cerr << '\'' << x << '\''; } void __print(const char *x) { cerr << '\"' << x << '\"'; } void __print(const string x) { cerr << '\"' << x << '\"'; } void __print(bool x) { cerr << (x ? "true" : "false"); } void __print(mint a) { cerr << a.x; } template <typename T, typename V> void __print(pair<T, V> x) { cerr << '{'; __print(x.first); cerr << ','; __print(x.second); cerr << '}'; } template <typename T> void __print(T x) { int f = 0; cerr << '{'; for (auto &i : x) cerr << (f++ ? "," : ""), __print(i); cerr << "}"; } template <typename T> void __print(stack<T> x) { vector<T> _x; while (!x.empty()) _x.push_back(x.top()), x.pop(); reverse(_x.begin(), _x.end()); __print(_x); } template <typename T> void __print(queue<T> x) { vector<T> _x; while (!x.empty()) _x.push_back(x.front()), x.pop(); reverse(_x.begin(), _x.end()); __print(_x); } template <typename T> void __print(priority_queue<T> x) { vector<T> _x; while (!x.empty()) _x.push_back(x.top()), x.pop(); reverse(_x.begin(), _x.end()); __print(_x); } template <typename T> void __print(priority_queue<T, vector<T>, greater<T>> x) { vector<T> _x; while (!x.empty()) _x.push_back(x.top()), x.pop(); reverse(_x.begin(), _x.end()); __print(_x); }
void _print() { cerr << "]\n\n"; }
...
template <typename U, typename T> T binser(U check, T _lo, T _hi, bool _minimize) { T _ret; while (_lo <= _hi) { T _mid = _lo + _hi >> 1ll; if (_minimize) { if (check(_mid)) { _hi = _mid - 1; _ret = _mid; } else { _lo = _mid + 1; } } else { if (check(_mid)) { _lo = _mid + 1; _ret = _mid; } else { _hi = _mid - 1; } } } return _ret; }
...
template <typename T> T pop(multiset<T> &que, bool front = 1) { assert(que.empty() == false); auto iterator = front ? que.begin() : prev(que.end()); T a = *iterator; que.erase(iterator); return a; }
template <typename T> T pop(set<T> &que, bool front = 1) { assert(que.empty() == false); auto iterator = front ? que.begin() : prev(que.end()); T a = *iterator; que.erase(iterator); return a; }
template <typename T> T pop(stack<T> &que) { T a = que.top(); que.pop(); return a; }
template <typename T> T pop(queue<T> &que) { T a = que.front(); que.pop(); return a; }
template <typename T> T pop(deque<T> &que, bool front) { T a = (front ? que.front() : que.back()); (front ? que.pop_front() : que.pop_back()); return a; }
template <typename T> T pop(priority_queue<T> &que) { T a = que.top(); que.pop(); return a; }
template <typename T> T pop(priority_queue<T, vector<T>, greater<T>> &que) { T a = que.top(); que.pop(); return a; }
template <typename T> T pop(vector<T> &que) { T a = que.back; que.pop_back(); return a; }
...
// void com_init(int N = M) { fact[0] = invfact[0] = 1; for (int i = 1; i < N; i++) fact[i] = fact[i - 1] * i; invfact[N - 1] = fact[N - 1].inv(); for (int i = N - 2; i; i--) invfact[i] = invfact[i + 1] * (i + 1); }
// template <typename T> mint C(T a, T b) { if (a < 0 || b < 0 || a < b) return 0; return fact[a] * invfact[b] * invfact[a - b]; }
 
// int primes[M / 10], lpf[M];
// void num_init() { int pc = 0; for (int i = 2; i < M; ++i) { if (!lpf[i]) primes[pc++] = i, lpf[i] = i; for (int j = 0; j < pc && 1LL * i * primes[j] < M && primes[j] <= lpf[i]; j++) lpf[i * primes[j]] = primes[j]; } }
// vector<pair<int, int>> pfact(int V) { vector<pair<int, int>> ret; while (V > 1) { int p = lpf[V], cnt = 0; while (V % p == 0) { V /= p; ++cnt; } ret.push_back({p, cnt}); } return ret; }
