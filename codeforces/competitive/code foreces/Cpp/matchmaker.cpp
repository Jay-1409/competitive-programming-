...
#define f0r(...)                                                               \
  overload5(__VA_ARGS__, FOR5, FOR4, FOR3, FOR2, FOR1)(__VA_ARGS__)
#define f0rd(...)                                                              \
  overload5(__VA_ARGS__, FOR5_R, FOR4_R, FOR3_R, FOR2_R, FOR1_R)(__VA_ARGS__)
...
#define recfun(type, name, ...)                                                \
  auto name = [&](auto &&name, __VA_ARGS__) -> type
...
template <typename T> void remDup(std::vector<T> &v) {
...
void solve() {
  int n, m;
  cin >> n >> m;
  vector<pair<int, int>> p, q, r, s;
  map<int, bool> m1, m2, n1, n2;
  int x, y;
  f0r(i, n) {
    cin >> x >> y;
    p.pb({x, y});
  ...
  f0r(i, n) {
    f0r(j, m) {
      if (p[i].second == q[j].second && m1[i] == false && m2[j] == false) {
        curr++;
  ...
  f0r(i, n) {
    f0r(j, m) {
      if (r[i].first == s[j].first && n1[i] == false && n2[j] == false) {
        cnt++;
        if (r[i].second == s[j].second) {
          ref++;
        }
        n1[i] = true;
        n2[j] = true;
...
int cnt[1000 + 1][1000 + 1], ask[100000 + 1][2];
...
int main() {
  ...
  map<int, int> mp;
  for (int i = 0; i < n; ++i)
    if (cnt[ask[i][0]][ask[i][1]]) {
      ++u;
      ++v;
      --cnt[ask[i][0]][ask[i][1]];
    } else if (!mp.insert(make_pair(ask[i][1], 1)).second)
      ++mp[ask[i][1]];
