...
void solve() {
    ...
    f0r(i,len(presa)){
        visited[presa[i]]=true;
    }
    f0r(i,len(presb)){
        visited[presb[i]]=true;
    }
    f0r(i,1,k+1){
        if(visited[i]==false){
            cout<<no<<endl;return;
    ...
    // cout<<len(presa)<<" "<<len(presb)<<endl;
    if(len(presa)<k/2||len(presb)<k/2){
        cout<<no<<endl;
    }else{
        f0r(i,k){
        if (find(presa.begin(), presa.end(), i) != presa.end() || find(presb.begin(), presb.end(), i) != presb.end()) {
