...
void dfs(int s,map<int,bool> &visited,vector<int> &adj) {
    if (visited[s]) return;
        visited[s] = true;
        f0r(s,len(adj)){
...
void solve() {
    ...
    f0r(i,n){
        ...
        finala.insert({v[i].first});
        finala.insert({v[i].second})
    }
    vector<int> adj(all(finala));
    // vector<pair<int,bool>>visited;
    map<int,bool> visited;
    f0r(i,n){
        visited[v[i].first]=false;
        visited[v[i].second]=false;
        // visited.push_back({v[i].first,false});
        // visited.push_back({v[i].second,false});
    }
    int u; cin>>u;
    dfs(u,visited,adj);
}
