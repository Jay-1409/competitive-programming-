...
void solve() {
    ...
    f0r(i,k){
        ...
        cin >> x >> y;
        arr[x][y] = true;
            if ((x > 1 && y > 1 && arr[x][y] && arr[x-1][y] && arr[x][y-1] && arr[x-1][y-1]) ||
            (x > 1 && y < m && arr[x][y] && arr[x-1][y] && arr[x][y+1] && arr[x-1][y+1]) ||
            (x < n && y > 1 && arr[x][y] && arr[x+1][y] && arr[x][y-1] && arr[x+1][y-1]) ||
            (x < n && y < m && arr[x][y] && arr[x+1][y] && arr[x][y+1] && arr[x+1][y+1])){
                ans = min(ans,i+1);
                flg = false;
            }         
    }
