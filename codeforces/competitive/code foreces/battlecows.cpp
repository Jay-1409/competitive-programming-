...
using namespace std;
// void vdel(vector<int> v,int x){
//         vector<int>::iterator it1, it2;
//     int valueToBeDeleted=x;
//         auto it = find(std::vector.begin(), vector.end(),
//                    valueToBeDeleted);
//     if (it != vector.end()) {
//         vector.erase(it);
//     }
...
void solve() {
    ...
    long long int n; sclld(n);
    long long int k; sclld(k);
    k--;
    long long int x; f0r(i,n){sclld(x);v.push_back(x);}
    long long int win=0;
    int flg =0;
    for(long long int i=0;flg==0;i++){
        if(v[k]>v[k+1]&&k!=n-1){
            v.erase(v.begin()+(k+1));
            win++;
        }else if(v[k]>v[k-1]&&k!=0){
            v.erase(v.begin()+(k-1));
            win++;
        }else {
            break;
