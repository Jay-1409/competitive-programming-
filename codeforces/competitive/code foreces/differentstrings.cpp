...
using namespace std;
bool fx1(int start,int end,int p){
    if(start<=end){
...
bool fx2(int a,int b,int c,int d){
    int x2=max(c,d); int str=min(a,b);
    int x1=max(a,b);
    int str2=min(c,d);
    return fx1(str,x1,str2)!=fx1(str,x1,x2)||fx1(str2,x2,str)!=fx1(str2,x2,x1);
}
void solve() {
    int a,b,c,d;
    cin>>a>>b>>c>>d;
