...
int findMedianBinaryArray(const std::vector<int>& nums) {
...
long long mod = 1e9+7;
int sum=0;
void printSubsequences(const std::vector<int>& vec, int k) {
    // if (k > vec.size() || k <= 0) {
    ...
    std::iota(indices.begin(), indices.end(), 0);
        ...
        int ans = findMedianBinaryArray(temp) % mod;
        sum+= ans;
