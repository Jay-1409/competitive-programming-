...
int W;
int dp[101][100001];
int kamui(int i,int ans,vector<int> & m,vector<int> &w,int n){
    if(i>=n-1){
        return 0;
    }
    if(dp[i][ans]!=-1){
    ...
    int res=kamui(i+1,ans,m,w,n);
    if(ans+w[i]<=W){
        res=w[i]+kamui(i+1,ans+w[i],m,w,n);
    }
    dp[i][ans]=max(ans,res);
    return max(ans,res);
}
void solve() {
