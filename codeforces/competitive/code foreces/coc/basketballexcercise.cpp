...
int level(int j, vector<int> &a, vector<int> &b, vector<vector<int>> &dp, bool choose) {
    if (j >= n) return 0; 
    if (dp[j][choose] != -1) return dp[j][choose]; 

    int result = 0;
    if (choose) {
        result = a[j] + level(j + 1, a, b, dp, false);
    } else {
        result = b[j] + level(j + 1, a, b, dp, true);
    }
...
void solve() {
    ...
    int result = max(level(0, a, b, dp, true), level(0, a, b, dp, false));
    cout << result << endl;
}
