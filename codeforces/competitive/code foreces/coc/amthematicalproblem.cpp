...
long long dp[25][25];
long long operation(long long a,long long b, char x){
    if(x=='+'){return a+b;}else return a*b;
} 
long long level(string str,int ref,int cnt){
    if(cnt==0)return stol(str.substr(ref));
    if(dp[ref][cnt]!=-1){
        return dp[ref][cnt];
    }
    long long side=1e10;
    f0r(i,ref,len(str)-cnt){
        long long a=stol(str.substr(ref,i-ref+1));
        long long b=level(str,i+1,cnt-1);
        side=min(side,operation(a,b,'+'));
        side=min(side,operation(a,b,'*'));
    }
    return dp[ref][cnt]=side;
}
void solve() {
    int n; cin>>n;
    string x; cin>>x;
    cout<<level(x,0,n-2)<<endl;
}
