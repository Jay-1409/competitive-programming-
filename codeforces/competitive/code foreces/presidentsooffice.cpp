...
void solve() {
    ...
    for (int i = 0; i < n - 1; ++i) {
        for (int j = 0; j < m; ++j) {
            if ((a[i][j] != a[i + 1][j]) && (a[i][j] != '.') && (a[i + 1][j] != '.') && ((a[i][j] == pr || a[i + 1][j] == pr))) {
                key[(int)a[i][j] + a[i + 1][j] - pr - 'A'] = true;
            }
        }
    }
    ...
    for (int i = 0; i < n ; ++i) {
        for (int j = 0; j < m - 1; ++j) {
            if ((a[i][j] != a[i][j + 1]) && (a[i][j] != '.') && (a[i][j + 1] != '.') && ((a[i][j] == pr || a[i][j + 1] == pr))) {
                key[(int)a[i][j] + a[i][j + 1] - pr - 'A'] = true;
            }
        }
    }
