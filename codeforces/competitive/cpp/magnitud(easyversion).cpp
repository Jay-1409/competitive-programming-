...
//     int neg=0;int zer=0;
//     f0r(i,n){cin>>v[i];if(v[i]<0)neg++;if(v[i]==0)zer++;}
//     long long int c=0;
//     if(neg+zer==n){
//         f0r(i,n){
//             c+=abs(v[i]);
...
//         //     c=c+v[i];
//         // }
//          c=max(abs(c+v[i]),c+v[i]);
//     }
//     }
//     cout<<c<<endl;
// }
int n; 
// int level(int i,vector<long long int> &v,vector<long long int> &dp){
//     if(i>=n-1){
//         return 0;
//     }
//     if(dp[i]<1e9+7){
...
//     int c=0;
//     dp[0]=abs(v[0]);
//     dp[i] = max(v[i] + level(i + 1, v, dp), abs(v[i]) + level(i + 1, v, dp));
//     return dp[i];
// }
...
void solve(){
    ...
    f0r(i,1,n+1){
            dx[i]=dx[i-1]+v[i];
            dp[i]=max(dp[i-1]+v[i],abs(dx[i-1]+v[i]));
    }
    cout<<dp[n]<<endl;
}
